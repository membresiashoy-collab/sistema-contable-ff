...
from modulos.coherencia_contable_componentes import _categoria_control_diagnostico
...
def test_ui_clasifica_alertas_por_tipo_de_control():
    assert (
        _categoria_control_diagnostico(
            {
                "codigo": "PLAN_CUENTAS_EMPRESA_IMPUTABLES_SIN_VINCULO_MAESTRO",
                "severidad": "ADVERTENCIA",
            }
        )
        == "Cuentas heredadas a sanear"
    )
    assert (
        _categoria_control_diagnostico(
            {
                "codigo": "PLAN_CUENTAS_EMPRESA_AGRUPADORAS_SIN_VINCULO_MAESTRO",
                "severidad": "INFO",
            }
        )
    ...
    assert (
        _categoria_control_diagnostico(
            {
                "codigo": "LIBRO_ASIENTOS_TRAZABILIDAD_HISTORICA_INCOMPLETA",
                "severidad": "INFO",
            }
        )
        == "Historial técnico / auditoría"
    )
    assert (
        _categoria_control_diagnostico(
            {
                "codigo": "ASIENTOS_PROPUESTOS_CUENTA_NO_RECONOCIDA",
                "severidad": "ERROR",
            }
        )
        == "Alertas accionables"
