...
def mostrar_ventas():
    ...
    # Botón para forzar la limpieza si ves ceros
    if st.button("⚠️ Resetear Base de Datos (Limpiar ceros)"):
        database.init_db()
    ...
    if archivo:
        try:
            ...
            if st.button("🚀 Procesar"):
                # Buscamos el último asiento para seguir la numeración
                ...
                for _, r in df.iterrows():
                    ...
                    # Lógica de Factura C o sin IVA
                    neto = total - iva if iva > 0 else total

                    # 1. DEBTOR (DEUDORES) AL DEBE
