            ...
            if st.button("🚀 Procesar Comprobantes"):
                ...
                for _, r in df.iterrows():
                    try:
                        ...
                        # LOGICA: Sin IVA, el total es venta plena
                        neto = total - iva if iva > 0 else total
                        es_rev = database.es_reverso(t)
                        
                        if es_rev: # NOTA DE CRÉDITO
                            ...
                            if iva > 0:
                                database.ejecutar_query("INSERT INTO libro_diario (id_asiento, fecha, cuenta, debe, haber, glosa, origen) VALUES (?,?,?,?,?,?,?)", (n_asiento, f, "IVA DEBITO FISCAL", 0, iva, f"Venta {t}", "VENTAS"))
                ...
                if cont > 0:
                    ...
                    st.success(f"✅ Se generaron {cont} asientos.")
                else:
                    st.warning("No se procesaron datos.")
